...
from wsd_krm_1_train import sens_prob, tag_types
...
fp_train_sorted_decision_list = codecs.open('krm_2_train_sorted_decision_list.txt','r','utf_8')  #* اشاره گر به فايل متني که ليست تصميم در آن نوشته مي شود
...
corpus_root = '/'
corpus_root = 'WSD/krm_4 folds_texts/krm_test/' # یا هر نسخه دیگر پایتون که بر روی دستگاه نصب شده) قرارگيرد)  python34 که شامل زیرفولدرهاي ذکر شده است بايد حتما در دايرکتوري WSD فولدر 
...
for i in range(peykare_lines_count):
    ...
    for j in range(len(dl_feature_types)):
        if appropriate_sens[i]=='':
            if dl_feature_types[j]=='krm_trif':
    ...
    if appropriate_sens[i]=='':
        for j in range(len(dl_feature_types)):
            if appropriate_sens[i]=='':
                if dl_feature_types[j]=='krm_plus1f' :
    ...
    if appropriate_sens[i]=='':
        for j in range(len(dl_feature_types)):
            if appropriate_sens[i]=='':
                if dl_feature_types[j]=='krm_minus1f' :
                    if (w_index!= 0):
                        if (line_words[w_index-1] == dl_feature_values[j]) or (dl_feature_values[j] in line_words[w_index-1]):
                            appropriate_sens[i]= dl_tag_values[j]
    ...
    if (appropriate_sens[i]==''):
        for j in range(len(dl_feature_types)):
            if appropriate_sens[i]=='':
                if dl_feature_types[j]=='krm_bif' and (dl_feature_values[j] in line_str):
                    appropriate_sens[i]= dl_tag_values[j]
...
print('krm_test_2 Precision =', Precision)
print()
