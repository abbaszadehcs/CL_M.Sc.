...
from wsd_shane_4_train import sens_prob, tag_types
...
fp_train_sorted_decision_list = codecs.open('shane_4_train_sorted_decision_list.txt','r','utf_8')  #* اشاره گر به فايل متني که ليست تصميم در آن نوشته مي شود
...
for i in range(peykare_lines_count):
    ...
    for j in range(len(dl_feature_types)):
        if appropriate_sens[i]=='':
            if dl_feature_types[j]=='shane_trif':
    ...
    if appropriate_sens[i]=='':
        for j in range(len(dl_feature_types)):
            if appropriate_sens[i]=='':
                if dl_feature_types[j]=='shane_plus1f' :
    ...
    if appropriate_sens[i]=='':
        for j in range(len(dl_feature_types)):
            if appropriate_sens[i]=='':
                if dl_feature_types[j]=='shane_minus1f' :
                    if (w_index!= 0):
                        if (line_words[w_index-1] == dl_feature_values[j]) or (dl_feature_values[j] in line_words[w_index-1]):
                            appropriate_sens[i]= dl_tag_values[j]
    ...
    if appropriate_sens[i]=='':
        for j in range(len(dl_feature_types)):
            if appropriate_sens[i]=='':
                if dl_feature_types[j]=='shane_bif' and (dl_feature_values[j] in line_str):
                    appropriate_sens[i]= dl_tag_values[j]
...
print('shane_test_4 Precision =', Precision)
print()
